...
export class FluxerPermissionMapper {
  ...
  rewrite(code: string): string {
    ...
    out = out.replace(
      /\bPermissionFlagsBits\.([A-Za-z0-9_]+)\b/g,
      "PermissionFlags.$1"
    );

    // 2. Convert permissionOverwrites.edit(role, { ... }) → channel.editPermission(role, { ... })
    out = out.replace(
      /([A-Za-z0-9_.]+)\.permissionOverwrites\.edit\s*\(\s*([^,]+)\s*,\s*({[^}]+})\s*\)/g,
      (match, channel, target, obj) => {
    ...
    out = out.replace(
      /([A-Za-z0-9_.]+)\.permissionOverwrites\.create\s*\(\s*([^,]+)\s*,\s*({[^}]+})\s*\)/g,
      (match, channel, target, obj) => {
        const converted = this.convertPermissionObject(obj);
        return `${channel}.editPermission(${target}, ${converted})`;
      }
  ...
  private convertPermissionObject(obj: string): string {
    ...
    for (const [, perm, value] of entries) {
      ...
      if (value === "true") allow.push(fluxerPerm);
      else deny.push(fluxerPerm);
    }
