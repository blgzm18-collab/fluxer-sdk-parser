...
exports.FluxerRoleMapper = void 0;
class FluxerRoleMapper {
    constructor(core) {
        this.core = core;
    }
    rewrite(code) {
    ...
    convertRoleObject(obj) {
        ...
        return (obj.substring(0, permStart) +
            `permissions: ${bitfield}` +
            obj.substring(bracketEnd + 1));
    }
    replaceRoleCreate(code) {
    ...
    replaceSetPermissions(code) {
        ...
        return code.replace(re, (_match, role, perms) => {
            const cleaned = perms
                .split(",")
                .map((p) => p.trim())
                .filter(Boolean)
...
exports.FluxerRoleMapper = FluxerRoleMapper;
