...
// src/SubpathMessage.ts
var SubpathMessage_exports = {};
__export(SubpathMessage_exports, {
  Message: () => Message
});
module.exports = __toCommonJS(SubpathMessage_exports);
...
function toReactionEmoji(emoji) {
  ...
  const parsed = (0, import_util3.parseEmoji)(emoji);
  if (parsed?.id) {
    return {
      name: parsed.name,
      id: parsed.id,
...
async function fetchMessageReactionUsersPage(message, emoji, options) {
  ...
  const route = import_types3.Routes.channelMessageReactionUsers(message.channelId, message.id, e) + (qs ? `?${qs}` : "");
