// src/index.ts
import { AttachmentBuilder, EmbedBuilder as EmbedBuilder4, MessagePayload as MessagePayload2 } from "@fluxerjs/builders";
import {
  AuditLogActionType,
  ChannelType as ChannelType5,
  ContentWarningLevel,
  GatewayOpcodes as GatewayOpcodes3,
  GuildMemberProfileFlags,
  GuildNSFWLevel as GuildNSFWLevel2,
  GuildOperations,
  InviteType as InviteType3,
  MessageAttachmentFlags,
  MessageFlags,
  MessageReferenceType as MessageReferenceType2,
  MessageType as MessageType2,
  OverwriteType as OverwriteType2,
  PublicUserFlags,
  RelationshipType,
  Routes as Routes35,
  SplashCardAlignment,
...
var GuildEmoji = class extends Base {
  ...
  async delete() {
    await this.client.rest.delete(Routes13.guildEmoji(this.guildId, this.id), {
      auth: true
    });
   ...
   */
  async edit(options) {
    const data = await this.client.rest.patch(Routes13.guildEmoji(this.guildId, this.id), {
      body: options,
      auth: true
    });
    this.name = data.name;
...
var GuildMemberRoleManager = class {
  ...
  async add(roleOrId) {
    ...
    await this.member.client.rest.put(
      Routes14.guildMemberRole(this.member.guild.id, this.member.id, id)
    );
    this.ids.push(id);
  }
  ...
  async set(roleIds) {
    const data = await this.member.client.rest.patch(
      Routes14.guildMember(this.member.guild.id, this.member.id),
...
var GuildMember = class _GuildMember extends Base {
  ...
  async edit(options) {
    ...
    const route = isMe ? Routes15.guildMemberMe(this.guild.id) : Routes15.guildMember(this.guild.id, this.id);
...
var GuildSticker = class extends Base {
  ...
  async delete() {
    await this.client.rest.delete(Routes16.guildSticker(this.guildId, this.id), {
      auth: true
    });
    const guild = this.client.guilds.get(this.guildId);
   ...
   */
  async edit(options) {
    const data = await this.client.rest.patch(Routes16.guildSticker(this.guildId, this.id), {
      body: options,
      auth: true
    });
    const s = data;
...
import {
  ...
  SnowflakeUtil as SnowflakeUtil5
} from "@fluxerjs/util";
var Role = class _Role extends Base {
  ...
  async edit(options) {
    ...
    const data = await this.client.rest.patch(Routes17.guildRole(this.guildId, this.id), {
      body: Object.keys(body).length ? body : void 0,
      auth: true
    });
    this._patch(data);
   ...
   */
  async delete() {
    await this.client.rest.delete(Routes17.guildRole(this.guildId, this.id), { auth: true });
...
var ChannelManager = class extends LimitedCollection {
  ...
  async fetch(channelId, options) {
    ...
    try {
      const data = await this.client.rest.get(Routes18.channel(channelId));
  ...
  async send(channelId, payload) {
    ...
    const data = await this.client.rest.post(
      Routes18.channelMessages(channelId),
      postPayload
    );
...
import { SnowflakeUtil as SnowflakeUtil6 } from "@fluxerjs/util";
var User = class extends Base {
   ...
   */
  async createDM() {
    const data = await this.client.rest.post(Routes19.userMeChannels(), {
...
export {
  ...
  DMChannel,
  EmbedBuilder4 as EmbedBuilder,
  ErrorCodes,
  Events,
  FluxerError,
  GatewayOpcodes3 as GatewayOpcodes,
  Guild,
  GuildBan,
  GuildChannel,
  GuildEmoji,
  ...
  GuildNSFWLevel2 as GuildNSFWLevel,
  GuildOperations,
  GuildSticker,
  Invite,
  InviteType3 as InviteType,
  LinkChannel,
  Message,
  MessageAttachmentFlags,
  MessageCollector,
  ...
  MessageFlagsBits,
  MessageManager,
  MessagePayload2 as MessagePayload,
  MessageReaction,
  MessageReferenceType2 as MessageReferenceType,
  MessageType2 as MessageType,
  OverwriteType2 as OverwriteType,
  PackManager,
  PermissionFlags4 as PermissionFlags,
  PermissionsBitField3 as PermissionsBitField,
  PublicUserFlags,
  ReactionCollector,
  RelationshipType,
  Role,
  ...
  resolveKlipyToImageUrl,
  resolvePermissionsToBitfield2 as resolvePermissionsToBitfield,
  serializeError,
