...
// src/SubpathCluster.ts
var SubpathCluster_exports = {};
__export(SubpathCluster_exports, {
  ...
  resetClientClusterBetaWarningForTests: () => resetClientClusterBetaWarningForTests
});
module.exports = __toCommonJS(SubpathCluster_exports);
...
function toRoleRequestBody(options) {
  ...
  if (options.permissions !== void 0) {
    body.permissions = typeof options.permissions === "string" ? options.permissions : (0, import_util3.resolvePermissionsToBitfield)(options.permissions);
  }
  return body;
}
...
async function fetchFileBuffer(url, index) {
  if (!URL.canParse(url)) {
    throw new import_util2.FluxerError(`Invalid file URL at index ${index}: ${url}`, {
      code: import_util.ErrorCodes.InvalidFileUrl
    });
  }
  let res;
  try {
    ...
    throw new import_util2.FluxerError(`Failed to fetch file from ${url}`, {
      code: import_util.ErrorCodes.FileFetchFailed,
      cause: err instanceof Error ? err : void 0
    });
  }
  if (!res.ok) {
    throw new import_util2.FluxerError(`Failed to fetch file from ${url}: ${res.status} ${res.statusText}`, {
      code: import_util.ErrorCodes.FileFetchFailed
    });
  }
...
async function resolveMessageFiles(files) {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if ("url" in f && f.url) {
      ...
      throw new import_util2.FluxerError(`File at index ${i} must have either "data" or "url"`, {
        code: import_util.ErrorCodes.InvalidAttachment
      });
    }
  }
  return result;
...
async function prepareMessagePostPayload(options, defaults) {
  ...
  if (typeof normalized === "string") {
    if (normalized.length === 0) {
      throw new import_util2.FluxerError("Cannot send an empty message", { code: import_util.ErrorCodes.EmptyMessage });
    }
  ...
  if (files?.length && uploadedAttachments?.length) {
    throw new import_util2.FluxerError("Cannot combine multipart files with uploadedAttachments", {
      code: import_util.ErrorCodes.InvalidMessageOptions
    });
  }
  if (replyTo && forward) {
    throw new import_util2.FluxerError("Cannot combine replyTo and forward", {
      code: import_util.ErrorCodes.InvalidMessageOptions
    });
  }
  ...
  if (forward) {
    if (!forward.channelId || !forward.messageId) {
      throw new import_util2.FluxerError("forward requires channelId and messageId", {
        code: import_util.ErrorCodes.InvalidMessageOptions
      });
    }
...
function httpStatus(err) {
  if (err instanceof import_rest2.FluxerAPIError) return err.statusCode;
...
function rethrowMapped(err, options) {
  ...
  if (err instanceof import_util2.FluxerError) throw err;
  if (options.notFound && httpStatus(err) === 404) {
    throw new import_util2.FluxerError(options.notFound.message, {
      code: options.notFound.code,
      cause: err
    });
  }
  throw new import_util2.FluxerError(errorMessage(err, options.fallback), {
    code: options.code,
    cause: err
  });
}
...
var MessageReaction = class extends Base {
  ...
  async fetchMessage() {
    try {
      ...
      rethrowMapped(err, {
        notFound: {
          code: import_util.ErrorCodes.MessageNotFound,
...
function validateBulkMessageFetchRequests(requests) {
  ...
  if (requests.length < req.min || requests.length > req.max) {
    throw new import_util2.FluxerError(`bulkFetchMessages requires between ${req.min} and ${req.max} requests`, {
      code: import_util.ErrorCodes.InvalidFetchLimit
    });
  }
  let total = 0;
  for (const request of requests) {
    if (request.limit < limit.min || request.limit > limit.max) {
      throw new import_util2.FluxerError(
  ...
  if (total > totalMax) {
    throw new import_util2.FluxerError(`bulkFetchMessages total message limit must not exceed ${totalMax}`, {
      code: import_util.ErrorCodes.InvalidFetchLimit
    });
  }
}
var MessageManager = class {
  ...
  async fetchMany(options) {
    if (options.limit != null && (options.limit < 1 || options.limit > 100)) {
      throw new import_util2.FluxerError("limit must be between 1 and 100", {
        code: import_util.ErrorCodes.InvalidFetchLimit
      });
    }
...
async function putBlob(url, body, contentType) {
  ...
  if (!res.ok) {
    throw new import_util2.FluxerError(`Presigned upload failed: ${res.status}`, {
      code: import_util.ErrorCodes.AttachmentUploadFailed
    });
  }
}
async function uploadAttachmentsForSend(client, channelId, files) {
  ...
  for (const item of plan.attachments) {
    ...
    if (!file) {
      throw new import_util2.FluxerError(`No file data for planned attachment id ${item.id}`, {
        code: import_util.ErrorCodes.InvalidAttachmentInput
      });
    }
  ...
  async bulkDelete(countOrIds) {
    ...
    if (typeof countOrIds === "number") {
      if (!Number.isInteger(countOrIds) || countOrIds < 1 || countOrIds > 100) {
        throw new import_util2.FluxerError("bulkDelete count must be between 1 and 100", {
          code: import_util.ErrorCodes.InvalidBulkDelete
        });
      }
      ids = [
    ...
    if (ids.length > 100) {
      throw new import_util2.FluxerError("bulkDelete requires at most 100 message IDs", {
        code: import_util.ErrorCodes.InvalidBulkDelete
      });
    }
...
var DMChannel = class extends TextCapable(Channel) {
  ...
  async purgeMessages() {
    if (this.type !== import_types10.ChannelType.DMPersonalNotes) {
      throw new import_util2.FluxerError("purgeMessages is only available on personal notes channels", {
        code: import_util.ErrorCodes.InvalidChannelType
      });
    }
    const data = await this.client.rest.post(
...
function requireString(obj, key, path) {
  ...
  if (typeof value !== "string" || value.length === 0) {
    throw new import_util2.FluxerError(`Invalid instance discovery: ${path}.${key} must be a non-empty string`, {
      code: import_util.ErrorCodes.InvalidInstanceDiscovery
    });
  }
  return value;
}
...
function parseInstanceDiscovery(raw) {
  if (!(0, import_util6.isRecord)(raw)) {
    throw new import_util2.FluxerError("Invalid instance discovery: expected an object", {
      code: import_util.ErrorCodes.InvalidInstanceDiscovery
    });
  }
  if (typeof raw.api_code_version !== "number" || !Number.isFinite(raw.api_code_version)) {
    throw new import_util2.FluxerError("Invalid instance discovery: api_code_version must be a number", {
      code: import_util.ErrorCodes.InvalidInstanceDiscovery
    });
  }
  if (!(0, import_util6.isRecord)(raw.endpoints)) {
    throw new import_util2.FluxerError("Invalid instance discovery: endpoints must be an object", {
      code: import_util.ErrorCodes.InvalidInstanceDiscovery
    });
  }
  const endpoints = {};
...
function resolveRestApi(instanceApi, restApi) {
  ...
  if (fromRest !== fromInstance) {
    throw new import_util2.FluxerError(
      ...
      { code: import_util.ErrorCodes.ConflictingInstanceConfig }
    );
  }
  return fromInstance;
}
...
function parseInviteCode(codeOrUrl) {
  ...
  if (!input) {
    throw new import_util2.FluxerError("Invite code cannot be empty", { code: import_util.ErrorCodes.InvalidInvite });
  }
  const fromUrl = (value) => {
  ...
  if (!code || /[\s/?#]/.test(code)) {
    throw new import_util2.FluxerError("Invalid invite code or URL", { code: import_util.ErrorCodes.InvalidInvite });
  }
  return code;
}
  ...
  requireToken() {
    if (!this.token) {
      throw new import_util2.FluxerError(
        ...
        { code: import_util.ErrorCodes.WebhookTokenRequired }
      );
    }
    return this.token;
  }
...
var User = class extends Base {
   ...
   */
  async createDM() {
    const data = await this.client.rest.post(import_types15.Routes.userMeChannels(), {
...
var GuildEmoji = class extends Base {
  ...
  get createdTimestamp() {
    return import_util10.SnowflakeUtil.timestampFromSnowflake(this.id);
  }
  /** Date when this emoji was created, derived from its snowflake ID. */
  get createdAt() {
    return import_util10.SnowflakeUtil.dateFromSnowflake(this.id);
  }
...
function computePermissions(basePermissions, overwrites, memberRoles, memberId, isOwner) {
  if (isOwner) return import_util11.ALL_PERMISSIONS_BIGINT;
  let perms = basePermissions;
  for (const overwrite of overwrites ?? []) {
    const applies = overwrite.type === import_types17.OverwriteType.Role && memberRoles.includes(overwrite.id) || overwrite.type === import_types17.OverwriteType.Member && overwrite.id === memberId;
    if (!applies) continue;
    const allow = BigInt(overwrite.allow || "0");
    const deny = BigInt(overwrite.deny || "0");
    perms = perms & ~deny | allow;
  }
  return (perms & import_util11.PermissionFlags.Administrator) !== 0n ? import_util11.ALL_PERMISSIONS_BIGINT : perms;
}
...
var GuildMemberRoleManager = class {
  ...
  async add(roleOrId) {
    ...
    await this.member.client.rest.put(
      import_types18.Routes.guildMemberRole(this.member.guild.id, this.member.id, id)
    );
    this.ids.push(id);
  }
  ...
  async set(roleIds) {
    const data = await this.member.client.rest.patch(
      import_types18.Routes.guildMember(this.member.guild.id, this.member.id),
...
var GuildMember = class _GuildMember extends Base {
  ...
  async edit(options) {
    ...
    const route = isMe ? import_types19.Routes.guildMemberMe(this.guild.id) : import_types19.Routes.guildMember(this.guild.id, this.id);
   ...
   */
  get permissions() {
    return new import_util12.PermissionsBitField(this._effectivePermissions([], []));
  }
  /**
  ...
  permissionsIn(channel) {
    return new import_util12.PermissionsBitField(
...
var GuildSticker = class extends Base {
  ...
  async delete() {
    await this.client.rest.delete(import_types20.Routes.guildSticker(this.guildId, this.id), {
      auth: true
    });
    const guild = this.client.guilds.get(this.guildId);
   ...
   */
  async edit(options) {
    const data = await this.client.rest.patch(import_types20.Routes.guildSticker(this.guildId, this.id), {
      body: options,
      auth: true
    });
    const s = data;
...
var Role = class _Role extends Base {
  ...
  get permissions() {
    ...
    return new import_util14.PermissionsBitField(
      (bits & import_util14.PermissionFlags.Administrator) !== 0n ? import_util14.ALL_PERMISSIONS_BIGINT : bits
    );
  }
  /**
   * Format as a mention string (`<@&id>`).
  ...
  async edit(options) {
    ...
    const data = await this.client.rest.patch(import_types21.Routes.guildRole(this.guildId, this.id), {
      body: Object.keys(body).length ? body : void 0,
      auth: true
    });
    this._patch(data);
   ...
   */
  async delete() {
    await this.client.rest.delete(import_types21.Routes.guildRole(this.guildId, this.id), { auth: true });
...
var ChannelManager = class extends import_collection6.LimitedCollection {
  ...
  async fetch(channelId, options) {
    ...
    try {
      const data = await this.client.rest.get(import_types22.Routes.channel(channelId));
      ...
      if (!channel) {
        throw new import_util2.FluxerError("Channel data invalid or unsupported type", {
          code: import_util.ErrorCodes.ChannelNotFound
        });
      }
  ...
  async send(channelId, payload) {
    ...
    const data = await this.client.rest.post(
      import_types22.Routes.channelMessages(channelId),
      postPayload
    );
...
async function resolveClientEmoji(client, emoji, guildId) {
  ...
  if (!parsed) {
    throw new import_util2.FluxerError("Invalid emoji", { code: import_util.ErrorCodes.InvalidEmoji });
  }
  if (parsed.id) {
  ...
  if (guildId) {
    ...
    throw new import_util2.FluxerError(
      ...
      { code: import_util.ErrorCodes.EmojiNotFound }
    );
  }
  if (/^\w+$/.test(parsed.name)) {
    throw new import_util2.FluxerError(
      ...
      { code: import_util.ErrorCodes.EmojiRequiresGuild }
    );
  }
  return parsed.name;
}
...
function assertChannelIdBatch(name, channelIds) {
  if (channelIds.length < 1 || channelIds.length > 100) {
    throw new import_util2.FluxerError(`${name} requires between 1 and 100 channel IDs`, {
      code: import_util.ErrorCodes.InvalidFetchLimit
    });
  }
}
...
async function fetchEmoji(guild, emojiId) {
  try {
    ...
    rethrowMapped(err, {
      notFound: { code: import_util.ErrorCodes.EmojiNotFound, message: `Emoji ${emojiId} not found in guild` },
...
async function fetchMember(guild, userId) {
  try {
    ...
    rethrowMapped(err, {
      notFound: {
        code: import_util.ErrorCodes.MemberNotFound,
...
async function fetchRole(guild, roleId2) {
  try {
    ...
    rethrowMapped(err, {
      notFound: { code: import_util.ErrorCodes.RoleNotFound, message: `Role ${roleId2} not found in guild` },
...
async function connectClientGateway(client, token, signal) {
  if (signal?.aborted) {
    throw new import_util2.FluxerError("Connection aborted", { code: import_util.ErrorCodes.GatewayConnectionAborted });
  }
...
var GuildManager = class extends import_collection10.LimitedCollection {
  ...
  async fetch(guildId, options) {
    ...
    try {
      ...
      rethrowMapped(err, {
        notFound: { code: import_util.ErrorCodes.GuildNotFound, message: `Guild ${guildId} not found` },
...
var Client = class _Client extends import_node_events3.EventEmitter {
  ...
  get ws() {
    if (!this._ws) {
      throw new import_util2.FluxerError("Client is not logged in. Call login() first.", {
        code: import_util.ErrorCodes.NotLoggedIn
      });
    }
    return this._ws;
  }
  ...
  requestGuildMembers(options) {
    ...
    if (!data.guild_id && !data.guild_ids?.length) {
      throw new import_util2.FluxerError("requestGuildMembers requires guildId or guildIds", {
        code: import_util.ErrorCodes.InvalidGatewayRequest
      });
    }
  ...
  async login(token, options) {
    if (this._ws) {
      throw new import_util2.FluxerError("Client is already logged in. Call destroy() first.", {
        code: import_util.ErrorCodes.AlreadyLoggedIn
      });
    }
    if (typeof token !== "string" || token.trim().length === 0) {
      throw new import_util2.FluxerError("Bot token is required.", { code: import_util.ErrorCodes.InvalidToken });
    }
  ...
  assertReady() {
    if (!this.isReady()) {
      throw new import_util2.FluxerError(
        ...
        { code: import_util.ErrorCodes.ClientNotReady }
      );
    }
  }
  static get Routes() {
...
function validateId(id) {
  if (typeof id !== "string" || id.trim().length === 0) {
    throw new import_util2.FluxerError("Runtime id must be a non-empty string.", {
      code: import_util.ErrorCodes.InvalidRuntimeConfig
    });
  }
  return id.trim();
}
function validateToken(token) {
  if (typeof token !== "string" || token.trim().length === 0) {
    throw new import_util2.FluxerError("Each runtime requires its own non-empty bot token.", {
      code: import_util.ErrorCodes.InvalidRuntimeConfig
    });
  }
  return token;
...
var ClientCluster = class extends import_node_events4.EventEmitter {
  ...
  *runtimeValues() {
  ...
  async add(input) {
    if (this.destroyed) {
      throw new import_util2.FluxerError("ClientCluster has been destroyed.", {
        code: import_util.ErrorCodes.ClusterDestroyed
      });
    }
    const id = validateId(input.id);
    const token = validateToken(input.token);
    if (this.runtimes.has(id) || this.pending.has(id)) {
      throw new import_util2.FluxerError(`Runtime id "${id}" is already in use.`, {
        code: import_util.ErrorCodes.DuplicateRuntimeId
      });
    }
    ...
    if (input.client?._ws) {
      throw new import_util2.FluxerError("Prebuilt client is already logged in. Pass an unconnected Client.", {
        code: import_util.ErrorCodes.RuntimeAlreadyLoggedIn
      });
    }
    ...
    const rt = this.runtimes.get(id);
    if (!rt) {
      throw new import_util2.FluxerError(`Runtime "${id}" failed to start.`, {
        code: import_util.ErrorCodes.RuntimeNotFound
      });
    }
  ...
  async restart(id, opts) {
    ...
    if (!existing) {
      throw new import_util2.FluxerError(`Runtime "${key}" was not found.`, {
        code: import_util.ErrorCodes.RuntimeNotFound
      });
    }
    if (existing.build.kind === "prebuilt") {
      throw new import_util2.FluxerError(
        ...
        { code: import_util.ErrorCodes.RuntimeConflict }
      );
    }
